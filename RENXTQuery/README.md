# RENXT Query Power BI Connector

This connector uses the [SKY API Query API](https://developer.sky.blackbaud.com/api#api=query) to run Raiser's Edge NXT saved queries and pull the results into Power BI. It is more efficient than the Blackbaud connector and can access larger datasets.
...
2. Open the `RENXTQuery` directory.
3. Update the `keys_client_id.txt` and `keys_client_secret.txt` files with values from [the application](https://developer.blackbaud.com/apps/) you registered in the Getting Started section.
4. Update the `keys_subscription_key.txt` file with the value from [SKY Developer Subscriptions](https://developer.blackbaud.com/subscriptions/).
5. Zip the contents of the `RENXTQuery` directory to create a `RENXTQuery.zip` file.
6. Rename `RENXTQuery.zip` to `RENXTQuery.mez`.
7. Verify that the `[Documents]\Power BI Desktop\Custom Connectors` directory exists.
8. Copy the `RENXTQuery.mez` file to the `[Documents]\Power BI Desktop\Custom Connectors` directory.

### Step 2 - Enable in Power BI Desktop

1. To enable use of uncertified custom data connectors, go to **File**, **Options and settings**, **Security** and under **Data Extensions**, enable **(Not Recommended) Allow any extension to load without validation or warning**.
2. Restart Power BI Desktop.
3. In Power BI Desktop, select **Get Data**, **Other**, **RENXT Query**.
...
Map the Data Gateway to the folder. You should see RENXT Query appear as an option for a custom connector.
...
6. Provide a data source name to represent the Connector, such as "RENXT Query."
...
For any questions and feedback related to this custom connector, use the [Blackbaud Community - Microsoft Power Platform category](https://community.blackbaud.com/forums/viewcategory/586).
